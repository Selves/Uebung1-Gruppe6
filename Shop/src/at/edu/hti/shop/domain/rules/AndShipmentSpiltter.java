...
public class AndShipmentSpiltter extends AbstractShipmentSplitter {

  private IShipmentSplitter one;
  private IShipmentSplitter other;

  public AndShipmentSpiltter(IShipmentSplitter one, IShipmentSplitter other) {
    this.one = one;
    this.other = other;
  }
  ...
  public boolean IsSatisfiedBy(List<OrderLine> orderLines, OrderLine orderToCompare) {
    return (one.IsSatisfiedBy(orderLines, orderToCompare) && other.IsSatisfiedBy(orderLines, orderToCompare));
  }
}
